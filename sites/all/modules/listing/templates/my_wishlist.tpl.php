  ...
  <script> 
   ...
   jQuery(document).ready(function() {  
	...
	jQuery('.search-basic').jRating({
		...
		onSuccess : function(elem,rate){
			jQuery.ajax({
				...
				data : {'type':'my_wishlist','rate':rate,'action':'filter'},
	...
	jQuery( "#slider-range" ).slider({
		range: true,
		min: <?php echo $minPrice['price']; ?>,
		max: <?php echo $maxPrice['price']; ?>,
		values: [ <?php echo $minPrice['price']; ?>, <?php echo $maxPrice['price']; ?> ],
	...
	?>
			...
			if(typeId.length != 0 && catID.length != 0){
				...
				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'catID':catID,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'search'};
				}
				else{
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'catID':catID,'typeId':typeId,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'search'};
				}
			}
			else if(typeId.length != 0 && catID.length == 0){

				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'search'};
				}
				else{
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'typeId':typeId,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'search'};
				}			
			}
			else if(typeId.length == 0 && catID.length != 0){
				dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'catID':catID,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'search'};
	...
	?>
			if(typeId.length != 0 && catID.length != 0){
				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','catID':catID,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'filter'};
				}
				else{
					dataString = {'type':'my_wishlist','catID':catID,'typeId':typeId,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'filter'};
				}
			}
			else if(typeId.length != 0 && catID.length == 0){
				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'filter'};
				}
				else{
					dataString = {'type':'my_wishlist','typeId':typeId,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'filter'};
				}
			}
			else if(typeId.length == 0 && catID.length != 0){
				dataString = {'type':'my_wishlist','catID':catID,'min':ui.values[ 0 ],'max':ui.values[ 1 ],'action':'filter'};
	...
	?>
			...
			var person = '<?php echo $_GET["search"]["person"]; ?>'
			if(this.value == 'all'){
				dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'min':min,'max':max,'action':'search'};
			}
			else{
				dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'typeId':this.value,'min':min,'max':max,'action':'search'};
			}
	<?php 
		}
		else{
	?>
			if(this.value == 'all'){
				dataString = {'type':'my_wishlist','min':min,'max':max,'action':'filter'};
			}
			else{
				dataString = {'type':'my_wishlist','typeId':this.value,'min':min,'max':max,'action':'filter'};
			}
	<?php 
		}
	?>	
	...
	jq("#edit-product-typeSelectBoxIt").addClass("dr-active");
		...
		?>
				...
				var person = '<?php echo $_GET["search"]["person"]; ?>'
				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'catID':this.value,'min':min,'max':max,'action':'search'};
				}
				else{
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'catID':this.value,'typeId':typeId,'min':min,'max':max,'action':'search'};
				}
		<?php 
			}
			else{
		?>
				dataString = {'type':'my_wishlist','catID':this.value,'typeId':typeId,'min':min,'max':max,'action':'filter'};
		<?php 
			}
		?>		
		jq.ajax({
...
function loadMoreData(offset){
	...
	?>
			...
			if(typeId.length != 0 && catID.length != 0){
				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'catID':catID,'min':min,'max':max,'action':'search','offset':offset};
				}
				else{
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'catID':catID,'typeId':typeId,'min':min,'max':max,'action':'search','offset':offset};
				}
			}
			else if(typeId.length != 0 && catID.length == 0){
			
				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'min':min,'max':max,'action':'search','offset':offset};
				}
				else{
					dataString = {'type':'my_wishlist','place':place,'when':when,'person':person,'typeId':typeId,'min':min,'max':max,'action':'search','offset':offset};
				}
			}
			else if(typeId.length == 0 && catID.length != 0){
				dataString = {'type':'my_wishlist','place':place,'type':etype,'when':when,'person':person,'catID':catID,'min':min,'max':max,'action':'search','offset':offset};
			}
			else{
				dataString = {'type':'my_wishlist','place':place,'type':etype,'when':when,'person':person,'min':min,'max':max,'action':'search','offset':offset};
			}
	<?php 
		}
		else{
	?>
			if(typeId.length != 0 && catID.length != 0){
				...
				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','catID':catID,'min':min,'max':max,'action':'filter','offset':offset};
				}
				else{
					dataString = {'type':'my_wishlist','catID':catID,'typeId':typeId,'min':min,'max':max,'action':'filter','offset':offset};
				}
			}
			else if(typeId.length != 0 && catID.length == 0){
				if(typeId == 'all'){
					dataString = {'type':'my_wishlist','min':min,'max':max,'action':'filter','offset':offset};
				}
				else{
					dataString = {'type':'my_wishlist','typeId':typeId,'min':min,'max':max,'action':'filter','offset':offset};
				}
			}
			else if(typeId.length == 0 && catID.length != 0){
				dataString = {'type':'my_wishlist','catID':catID,'min':min,'max':max,'action':'filter','offset':offset};
  ...
  jQuery(".inlinepop").colorbox({inline:true, width:"40%", href:"#login-pop"});
	...
	function updateViewsCount(listingID){
		jQuery.ajax({
			...
			success: function(data){
				location.href='<?php echo $base_url; ?>/experience/view/'+listingID;
			},
			error: function(data){
				alert("ajax error occured…"+data);
			}
