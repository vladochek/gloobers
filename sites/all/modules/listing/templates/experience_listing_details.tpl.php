...
<script src="<?php print $GLOBALS['base_url'].'/'. drupal_get_path('theme',$GLOBALS['theme']); ?>/js/jquery.classyloader.min.js" type="text/javascript"></script>
<?php
...
<img class="image_print" style="display:none" src="<?php print image_style_url($FBstyle, $imgpath); ?>" alt="" title="" />
<?php }else{
$imgpath=$Img_uri;
...
//BookNow Popup
	jQuery(".booknowbtn").click(function(){
		if(jQuery('.totalamount').find('h4')[0].innerHTML=='$0'){
			...
			jQuery.ajax({
				...
				type: 'post',
				data:{'date':selectedDate,'lid':listID,'availiblitymax':availiblitymaxval,'availiblitymin':availiblityminval,'total':totalselected,'session_time':session_time},
	...
	jQuery("#copy-dynamic1").on("click",function(){
			...
			jQuery('#copy-dynamic1').zclip(
           {
               ...
               copy: function(){ return jQuery('#dynamic1').val(); },
               afterCopy: function()
               {
                  console.log("success");
...
<input type="search" name="search_desti" id="experience_loction" value="<?php print $search_dest_bar; ?>" class="desinput" onKeyup="search_destination();" onblur="if (this.placeholder == ''){this.placeholder = 'Enter a destination'; }" onfocus="if(this.placeholder == 'Enter a destination'){this.placeholder = '';}" placeholder="Enter a destination" />
...
<li class="active"><a aria-expanded="true" aria-controls="photos" data-toggle="tab" role="tab" id="photos-tab" href="#photos">Photos</a></li>
<!-- <li><a aria-controls="Calendar" data-toggle="tab" id="Calendar-tab" role="tab" href="#Calendar" aria-expanded="false">Calendar</a></li> -->
<li><a aria-controls="Map" data-toggle="tab" id="Map-tab" role="tab" href="#Map" aria-expanded="false">Map</a></li>
</ul>    
</div> <!-- End Tabs -->
...
if(!empty($photos)){
	...
	foreach ($photos as $photo){
		...
		if($countA==1){$activeClass='active';}else{$activeClass='';}
		?>
		<div class="item <?php echo $activeClass; ?>">
		<img onerror="this.onerror=null;this.src='<?php echo $base_url . '/' . drupal_get_path('theme', $GLOBALS['theme']) . '/images/default_exp_banner.jpg'; ?>'" src="<?php print image_style_url($style, $imgpath); ?>" alt="" title="" />
		</div>
		<?php
		$countA++;
	}
}else{ ?>
	<div class="item active"> <img src="<?php echo $base_url . '/' . drupal_get_path('theme', $GLOBALS['theme']) . '/images/cover_pic.jpg'; ?>"  />
	</div>
<?php 
}
?>
...
<a class="left carousel-control white_left_arrow" href="#hotelslider" data-slide="prev" onclick="jQuery('#hotelslider').carousel('prev')" ></a>
<a class="right carousel-control white_right_arrow" href="#hotelslider" data-slide="next" onclick="jQuery('#hotelslider').carousel('next')"></a>
<?php } ?>
...
<?php 
$listing_addr='';
if($OverviewData['city']){$listing_addr .= ucfirst($OverviewData['city']). ', ';}
if($OverviewData['state']){$listing_addr.=ucfirst($OverviewData['state']). ', ';}
if($OverviewData['country']){$listing_addr.=ucfirst($OverviewData['country']);}
print $listing_addr; ?>
</h4>
...
<div class="mappanel" id="map-canvas"> <div class="mapframe"> Loading Map view please wait.... </div> </div>
</div> <!-- End Map TabPane -->

<div aria-labelledby="Calendar-tab" id="Calendar" class="tab-pane fade" role="tabpanel">
<div class="calenpanel"> <div id="calendar_full">Loading calendar please wait....</div> </div>
...
<p>Check In</p>
<input type="text" id="avail-dates" autocomple="false" data-date-format="YYYY/MM/DD" name="date" placeholder="Select Date" readonly="readonly" value="" />
...
<p><i><img src="<?php echo $base_url.'/'.drupal_get_path('theme', $GLOBALS['theme']); ?>/images/recommend_icon.png" alt="" title="" /></i></p>
...
<div class="col-md-4 col-sm-4 col-xs-12">
<h3>Reviews</h3>
<span class="hotelstar"><a class="reviewsCountAnchr">(<?php echo sizeof($reviews); ?> reviews)</a></span>
</div> <!-- End Col -->
...
$i=1; foreach($categoriesCount as $key=>$value){ if($i<5) { $loader="loader_".$i;  ?>
			
				<div class="canvas-outr"><canvas class="loader_common" id="<?php echo $loader ;?>"></canvas>

				<?php 
				$RecPassion=(($value/sizeof($reviews))*100);
				//$RecPassion=sprintf('%.2f',$RecPassion); 
				?>
				<p style="display:none"><?php echo round($RecPassion);?></p>			
				<p>
				<?php 
				$catName=getPassionCategoryList($key);
				$catnameCount = strlen($catName[0]['passion_category']);
				echo $catName[0]['passion_category'];
				?>
				</p>
				</div>					
...
if ($extras){ ?>
	...
	if($extras['price-type']=='%'){ $percnt='%';$dolr=''; }else{ $percnt='';$dolr='$';}
	if($extras['price-key']=='per-order'){ $orderType='Order';}else{ $orderType='Quantity';}
	?>
	<div class="col-md-12">
	<div class="col-md-10 col-sm-10 col-xs-12">
	<input id="service-<?php echo $value["meta_id"];?>" class="css-checkbox" type="checkbox" onclick="getdefaultCounter(this);"  attr-value="optional-select-<?php echo $value["meta_id"]; ?>" />
	<label for="service-<?php echo $value["meta_id"] ?>" class="css-label" ><?php echo $extras["title"]; ?> (<?php echo $dolr.$extras["price"].$percnt.'/'.$orderType; ?>)</label>
	</div>
	<div class="col-md-2 col-sm-2 col-xs-12">
	<div class="input-group">
			  <span class="input-group-btn">
				  <button type="button" class="btn btn-default btn-number" id="minus-<?php echo $quantkey;?>"  onclick="getCounter(this);" disabled="disabled" data-type="minus" data-field="optional-select-<?php echo $value["meta_id"]; ?>">
					  <span class="glyphicon glyphicon-minus"></span>
				  </button>
			  </span>
			  <input type="text" name="optional-select-<?php echo $value["meta_id"]; ?>" readonly attr-value="<?php echo $value["meta_id"]; ?>" id="<?php echo 'opqty_'.$quantkey; ?>" class="form-control input-number optional-services-dropbox" value="1" min="1" max="10" data-value="1"/>
			  <span class="input-group-btn">
				  <button type="button" class="btn btn-default btn-number" id="plus-<?php echo $quantkey;?>" data-type="plus" onclick="getCounter(this);" data-field="optional-select-<?php echo $value["meta_id"]; ?>">
...
?>
<p><?php echo trim($userDetails->field_first_name['und'][0]['value'],"%20"); ?></p><a href="<?php echo $base_url.'/profile/'.$userDetails->uid; ?>">View full profile</a>
</div>
...
<div class="progress">
<div class="value"><i class="fa fa-map-marker"></i> <?php echo $first_refundAmountPercent; ?></div>
...
<div class="progress">
<div class="value"><i class="fa fa-map-marker"></i> <?php echo $second_refundAmountPercent; ?></div>
                        ...
                        <div class="tab-content">
                            ...
                            <?php $gmail_link=explode('&', $Listing_recommend_link); ?>
                                <form>
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" readonly value="<?php echo $Listing_recommend_link; ?>" id="recipient-name">
										<div class="social_tabs_popup_link">
										<a target='_blank' class="fa fa-twitter" target="_blank" href="https://twitter.com/home?status=<?php echo $Listing_recommend_link; ?>"></a>
										<a target='_blank' class="fa fa-envelope-o" href="mailto:?subject=You%20received%20a%20new%20recommendation%20from%20Gloobers&body=Hi,%0D%0A%0D%0AYou%20just%20received%20a%20new%20recommendation!%0D%0A%0D%0AClick%20on%20the%20following%20link%20or%20sign%20up%20on%20http://gloobers.com%20to%20view%20this%20recommendation.%0D%0A%0D%0A<?php echo $gmail_link[0]; ?>%0D%0A%0D%0ACheers,%0D%0A%0D%0AThe%20Gloobers%20team"></a>
										<a target='_blank' class="fa fa-google-plus" href="https://plus.google.com/share?url=<?php echo $Listing_recommend_link;?>"></a>
										<a target='_blank' class="fa fa-facebook" href="https://www.facebook.com/sharer/sharer.php?u=<?php echo $Listing_recommend_link;?>"></a>
                            ...
                            <div class="tab-pane active" id="tab2">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="submit" class="btn btn-primary btnsend" onclick="addListingToTravelguide();">Add</button>
                                </div>
                            ...
                            <div class="tab-pane" id="tab3">
                                <form>
                                    <div class="form-group">
                                        <label for="message-text" class="control-label">Copy Listing's Embed Code</label>
                  <textarea readonly class="form-control" id="message-text"><a class="embedly-card" href="<?php echo $Listing_recommend_link; ?>">Gloobers</a>
<script async src="<?php $base_url.'/'.drupal_get_path('theme', $GLOBALS['theme']); ?>/js/platform.js" charset="UTF-8"></script></textarea>
                                    </div>
                                </form>
                            </div>
                        </div>
...
<div class="modal fade" id="fullCalenderModal11" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content loginpop">
            ...
            <div class="modal-footer" style="text-align: right;">        
                <p style="float: right;"><button href="#" class="btn btn-success" id="updateFullCalenderEventSession">Done</button></p>
            </div>
        </div>
    </div>
</div>
...
<div class="popuppanelblock"><span class="recomendation_close_btn cancelbtn">X</span><div class="popuppanelblock_inner">
<h1>Recommend this listing</h1>
<div class="paratext">
<p>You are about to recommend this listing to a friend. You will be notified as soon as your friend follows your recommendation and rewarded after he will execute his trip.</p>
</div> <!-- End ParaText -->
...
<div class="col-md-10 col-sm-10 col-xs-12">
<div class="clipboard">
<input name="input" id="dynamic1" readonly="" type="text" value="<?php echo $Listing_recommend_link; ?>">
</div>
</div>
<div class="col-md-2 col-sm-2 col-xs-12">
<a class="actionbtn btn" data-clipboard-action="copy" data-clipboard-target="#dynamic1" id="copy-dynamic1" href="javascript:void(0);">Copy</a>
...
<div class="recommendlinkblock">
	<div class="social_tabs_popup_link">
		<a target='_blank' class="fa fa-twitter" target="_blank" href="<?php echo $twitter_recommend_link; ?>"></a>
...
<p>Embed this recommendation onto your website or blog, get rewarded for every booking you generate.</p>
...
<a href="<?php echo $Listing_recommend_link; ?>"><img src="<?php echo $FBsrc; ?>" style="width:100%;border:0;outline:0;margin-bottom:10px" alt="" title="">
...
<div class="paratext">
<p>You are about to book this listing. You can use your gloobies to get an exclusive discount. Choose the amount of gloobies you want to use and confirm your reservation. Your credit card details may be needed for deposit purposes.
</div> <!-- End ParaText -->
...
<div class="col-md-6 col-sm-6 col-xs-6"><label>Gloobers service fees(<?php echo $service_tax.'%'; ?>)</label></div>
<div class="col-md-6 col-sm-6 col-xs-6" id="service_fees"><h4></h4></div>
</div> <!-- End AmountFields -->

<div class="amountfields">
<div class="col-md-6 col-sm-6 col-xs-6"><label>Available gloobies(<?php echo $totalCredits;?>)</label></div>
<div class="col-md-6 col-sm-6 col-xs-6"><h4 class="colorblue">$<?php echo $availble_gloobies; ?></h4></div>
</div> <!-- End AmountFields -->
<?php ?>
<div class="amountfields">
<div class="col-md-6 col-sm-6 col-xs-6"><label>Pay in gloobies</label></div>
<div class="col-md-6 col-sm-6 col-xs-6 no_padding">
<input type="text" name="payin" class="payableamount" id="minMax" value="" onkeyup="this.value = minmax(this.value, 0,'<?php echo $availble_gloobies; ?>')" /></div>
</div> <!-- End AmountFields -->
</div> <!-- End Row -->
</div> <!-- End Col -->
...
<div class="col-md-6 col-sm-6 col-xs-6" id="Pending_credits_amt"><h4><strong><?php echo '$'.$availble_gloobies; ?></strong></h4></div>
</div> <!-- End AmountFields -->
</div> <!-- End AmountCreaditBlock -->

<div class="payableBtns">
<div class="col-md-9 col-md-offset-3">
<input type="submit" name="confirm" onclick="return confirmCredits();" value="Confirm" class="totalpayee" />
<span class="cancelbtn">Cancel</span>
</div> <!-- End Col -->
</div> <!-- End PayableBtns -->
...
<script type="text/javascript">
	function getBookingQuantity(){		
		...
		jQuery.ajax({
            ...
            type: 'post',
			async:false,
			data: {selected_booking_time:selected_booking_time,selected_booking_date:selected_booking_date,listing_id:listing_id},
			success: function (data) {
				//alert(data);
				console.log(data);console.log(qty_Max);console.log(qty_Max);
	...
	function getPricing_newData(timechange){
		...
		jQuery.ajax({
            ...
            type: 'post',
			async:false,
			data: {selected_booking_time:timepickerVal,selected_booking_date:selected_booking_date,listing_id:listing_id,time_changes:time_changes_var},
	...
	function get_travellers_new(priceType,price,quantkey,adult_max,child_max,childMin){
		...
		jQuery.ajax({
				...
				type: 'post',
				async:false,
				data: {quantity:optionSelected_modify,persontype:personType,priceType:priceType,calculatedPrice:calculatedPrice,person_qty_adult:person_qty_adult,person_qty_child:person_qty_child,status:1,adult_max:adult_max_val,child_max:child_max_val,price:actual_price,occupancy_max:occupancymax,status_new_val:status_new},
				success: function (data) {
					var data = jQuery.parseJSON(data);
	...
	function get_travellers_new1(priceType,price,quantkey,adult_max,child_max,adultMin,childMin){
		...
		jQuery.ajax({
				...
				type: 'post',
				async:false,
				data: {quantity:optionSelected_modify,persontype:personType,priceType:priceType,calculatedPrice:calculatedPrice,person_qty_adult:person_qty_adult,person_qty_child:person_qty_child,status:1,price:actual_price,item_boxcase:item_box_case},
				success: function (data) {
					var data = jQuery.parseJSON(data);
					var mainSubtotal=data.Mainsubtotal;
					jQuery('.totalamount').find('h4').html('$'+mainSubtotal);
	...
	function calculateOfferTotal(){
		...
		jQuery.ajax({
            ...
            success: function (data){
				...
				if(data.disscountBy=='$'){var datediss=data.disscountBy;}else{var datediss=data.disscountBy+' ($'+data.disscountedAmount+')';}
   ...
   function optionalServices(obj) {
		...
		for(i=1;i<=personPriceSize;i++){
			...
			if(getvalarr < 0){
				...
				for(option=1;option<=totalqty;option++){
					if(option==selectedVal){var selected='selected';}else{var selected='';}
	...
	function confirmCredits(){
		...
		jQuery.ajax({
				...
				type: 'post',
				data: {Pending_credits_amt:Pending_credits_amt,available_gloobies:available_gloobies,used_gloobies:used_gloobies},
				success: function (data){
					window.location.href=$baseUrl+'/booking/summary';
				}
