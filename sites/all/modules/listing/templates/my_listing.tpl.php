    	...
    	<div class="dr-full-width-wrapper">
         ...
         <div class="dr-full-width-wrapper">
         	<div class="dr-result-container">
				...
				?>
                    	<h2><?php echo $_GET['search']['place']; ?><span><?php echo ($_GET['search']['type'] != '')?' ,'.substr($_GET['search']['type'],0,(strlen($_GET['search']['type'])-2)):''; ?></span></h2>
         ...
         <div class="tab-content-wrap">
				   ...
				   ?>
                    ...
                    <div class="dr-pricing">
						...
						?>
                                ...
                                <h4><a href="<?php print $base_url; ?>/experience/view/<?php echo $listing["eid"]; ?>"><?php echo ucwords($listing["title"]); ?></a></h4>
			...
			?> 
					<span class="no-result">No listing added by you</span>
			<?php 
				}
			?>
            </div><!-- row ends --> 
   ...
   jQuery(document).ready(function() {  
	...
	jq(document).on('change','#edit-product-category', function() {
		...
		jq.ajax({
			...
			data : {'catID':this.value,'typeId':typeId,'min':min,'max':max,'action':'filter'},
			success: function(data){
				data = jq.parseJSON(data);
				if(data.count == 0){
					jq(".result-row").html("<span class='no-result'>No results found</span>");
...
function scrollMore(){
	...
	if(jQuery(window).scrollTop() == (jQuery(document).height() - jQuery(window).height())){
		...
		jQuery("#loaderImg").html('<img src="<?php print base_path() . drupal_get_path('theme', 'gloobers'); ?>/images/ajax-loader.gif" />');
