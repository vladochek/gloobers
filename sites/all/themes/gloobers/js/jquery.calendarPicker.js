...
jQuery.fn.calendarPicker = function(options) {
  ...
  if (typeof(options.callbackDelay) == "undefined")
	...
	if (typeof(options.disableDays) == "undefined")
    options.disableDays=new Array();
  ...
  calendar.changeDate = function(date) {
    ...
    var fillDays = function(date) {
      ...
      for (var i = -options.days; i <= options.days; i++) {
        ...
        d.setDate(day + i);
		if(d.getDate()<10){
			var cdate = '0'+d.getDate();
		}
		else{
			var cdate = d.getDate();
		}
		if(d.getMonth()<9){
			var cmonth = '0'+(d.getMonth()+1);
		}
		else{
			cmonth = '';
			cmonth = parseInt(d.getMonth()+1);
		}
		var monthdate = cdate + "-" + cmonth + "-" + d.getFullYear();
		if(options.disableDays.indexOf(monthdate) > -1)
		{	
