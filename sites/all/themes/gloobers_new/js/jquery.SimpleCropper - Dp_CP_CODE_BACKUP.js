...
(function ($) {
    ...
    $.fn.simpleCropper = function (onComplete) {
        ...
        function preview() {
			...
			var img_cover=$('#cp').find('img').attr('src');
			var img_profile=$('#dp').find('img').attr('src');
			if(img_cover){
					img_cp="<input type='hidden' name='cp' class='Cp' value="+img_cover+"/>"
					$('#cp_pic').html(img_cp);
			
			}
			if(img_profile){
				img_dp="<input type='hidden' name='dp' class='Dp' value="+img_profile+"/>"
					$('#dp_pic').html(img_dp);
			}
