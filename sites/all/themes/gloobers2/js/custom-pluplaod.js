...
jq(".form-type-plupload").plupload({
            ...
            FileUploaded: function(up, file, response) {
                ...
                jQuery('.plupload_content').find('.plupload_filelist_content').find('#'+file.id).attr('file-id', response);
		...
		/* return false; */
			...
			jConfirm("Are you sure you wish to delete this photo? It's a nice one! ", 'Delete Photo', function(r) {
				if (r == true)
				{
					jq.ajax({
						beforeSend: function(){
							if(jq( ".ajax-running" ).length>0){
							jq(".photos-wrapper").find(".ajax-running").html('Deleting...');
							jq(".photos-wrapper").find(".ajax-running").show();
							}else{				
						...
						complete: function(){
							jq("#"+menuchahidihaiid).parent().find(".ajax-running").delay(1500).fadeOut('slow');
					...
					.done(function( data ) {
						...
						if(jq( ".ajax-running" ).length>0){
							jq(".photos-wrapper").find(".ajax-running").html('Deleted');
							jQuery(".photos-wrapper").find(".ajax-running").show();
						} else{
		...
		/*updated 2 june 2016*/
			...
			jq( ".highlights" ).blur(function() {	
				...
				jq.ajax({
					beforeSend: function(){
						if(jq( ".ajax-running" ).length>0){
							jq(".photos-wrapper").find(".ajax-running").html('Saving...');
							jq(".photos-wrapper").find(".ajax-running").show();
						}else{
					...
					complete: function(){
						jq("."+menuchahidihaiid).parent().find(".ajax-running").delay(2000).fadeOut('slow');
					},
					type: "POST",
				...
				.done(function( data ) {
					...
					if(jq( ".ajax-running" ).length>0){
						jq(".photos-wrapper").find(".ajax-running").html('Saved');
						jQuery(".photos-wrapper").find(".ajax-running").show();
					}else{
						jq("."+menuchahidihaiid).parent().append('<span style="color:green;font-size:15px;font-weight:bold;" class="ajax-running">Saved</span>');
			...
			/*if((navigator.userAgent.match(/iPad/i)) && (navigator.userAgent.match(/iPad/i)!= null)){
		 			 ...
		 			 jQuery('.add_photo_content').find('.ui-button-text').on('click',function(){
           				 jQuery('.plupload_filelist_footer').find('input').trigger('click');
            		 });
				}*/
